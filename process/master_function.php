...
if($process == "add")
{
	...
	$b_data['WFS_CONSTRAINT_TYPE'] = "PK";
	$b_data['WFS_ORDER'] = "1";
	$b_data['WFS_FIELD_NAME'] = conText($_POST['WFS_FIELD_NAME']);
	$b_data['WFS_FIELD_TYPE'] = $data_type;
	$b_data['WFS_FIELD_LENGTH'] = $data_length;
	$b_data['MM_ID'] = $mm_id;

	//db::db_insert("MASTER_STEP", $b_data);

	create_table($WF_MAIN_SHORTNAME, conText($_POST['WF_FIELD_PK']), $data_type, $data_length);
}
elseif($process == "edit")
{
	if($_FILES["UPLOAD_FILE"]["size"]>0)
