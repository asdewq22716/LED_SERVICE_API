...
$s_system_name = array('W'=>'Workflow','M'=>'Master','F'=>'Form');
?>
		...
		<div class="container-fluid">
			...
			<div class="row" id="animationSandbox">
				<div class="col-sm-8">
					<div class="main-header">
						...
						<ol class="breadcrumb breadcrumb-title breadcrumb-arrow">
							...
							<li class="breadcrumb-item">
								<a href="#"><?php echo $txt_system_sub; ?></a>
							</li>
						</ol>
					</div>
				</div>
				<div class="col-sm-4">
					...
					<div class="f-right">
						<nobr>
						<a class="btn btn-primary waves-effect waves-light" href="<?php echo $txt_link_add; ?>" role="button">
							<i class="icofont icofont-ui-add"></i> เพิ่มข้อมูล
						</a>
						<?php if($WF_TYPE == "W"){ ?> <a class="btn btn-primary waves-effect waves-light" href="sa_add_flow.php" role="button">
							<i class="icofont icofont-ui-add"></i> Wizard
						</a> 
						<?php } if($WF_TYPE == "M"){ ?>
						<a class="btn btn-success waves-effect waves-light" href="master_import.php" role="button">
							<i class="icofont icofont-file-excel"></i> นำเข้า Excel
						</a>
						<a class="btn btn-info waves-effect waves-light" href="master_import_table.php" role="button">
							<i class="icon-login"></i> Import table 
						</a>
						<?php } ?>
						</nobr>
					</div>
			...
			{
				$sql_main = db::query("select * from WF_MAIN where WF_GROUP_ID = '".$rec_group['GROUP_ID']."' AND WF_TYPE = '".$WF_TYPE."' ORDER BY WF_MAIN_ORDER ASC");
			?>
					...
					<div class="col-md-12">
						<div class="card">
							<div class="card-header">
								...
								<div class="f-right">
									<a href="#primary" class="btn btn-warning waves-effect waves-light" onclick="save_wf_order();" role="button">
										...
										?>
											<tr class="wf_keyword-box" id="tr_main<?php echo $rec_main['WF_MAIN_ID'];?>">
												...
												<td class="text-center">
													<nobr>
														<button type="button" class="btn btn-warning btn-icon" data-toggle="tooltip"  title="แก้ไข <?php echo $s_system_name[$WF_TYPE];?>" onclick="window.location.href='<?php echo $txt_link_edit; ?>?W=<?php echo $rec_main['WF_MAIN_ID']; ?>'">
															<i class="icofont icofont-edit-alt"></i>
														</button> &nbsp;
														<?php if($WF_TYPE == "W"){ ?>
														<button type="button" class="btn btn-success btn-icon" data-toggle="tooltip"  title="เพิ่ม/แก้ไข Step" onclick="window.location.href='workflow_detail.php?W=<?php echo $rec_main['WF_MAIN_ID']; ?>'">
															<i class="icofont icofont-settings"></i>
														</button> &nbsp;
														<?php }elseif($WF_TYPE != "R"){ ?>
														<button type="button" class="btn btn-success btn-icon" data-toggle="tooltip"  title="ตั้งค่า Field" onclick="window.location.href='<?php echo $txt_link_field; ?>?W=<?php echo $rec_main['WF_MAIN_ID']; ?>&WFD=0'">
															<i class="icofont icofont-settings"></i>
														</button> &nbsp;
														<?php } ?>
														<?php if($WF_TYPE == "R" AND $rec_main['WF_MAIN_TYPE'] == "D"){ ?>
														<button type="button" onclick="window.location.href='report_setting.php?W=<?php echo $rec_main['WF_MAIN_ID']; ?>';" class="btn btn-success btn-icon" data-toggle="tooltip"  title="บริหาร Dashboard" role="button">
															<i class="icofont icofont-chart-bar-graph"></i>
														</button> &nbsp;
														<?php } ?>
														<?php if($WF_TYPE == "M"){ ?>
														<button type="button" onclick="window.open('master_main.php?W=<?php echo $rec_main['WF_MAIN_ID']; ?>','','');" class="btn btn-primary btn-icon" data-toggle="tooltip"  title="บริหารข้อมูล" role="button">
															<i class="fa fa-table"></i>
														</button> &nbsp;
														<?php } ?>
														<button type="button" class="btn btn-info btn-icon" data-toggle="tooltip"  title="ตั้งค่าการค้นหา" onclick="window.location.href='search_step_form.php?W=<?php echo $rec_main['WF_MAIN_ID']; ?>&WFD=0'">
															<i class="icofont icofont-search-alt-2"></i>
														</button> &nbsp;
														<button type="button" class="btn btn-primary btn-icon" data-toggle="tooltip"  title="Copy <?php echo $s_system_name[$WF_TYPE];?>" onclick="window.location.href='copy_workflow.php?W=<?php echo $rec_main['WF_MAIN_ID']; ?>&WF_TYPE=<?php echo $WF_TYPE; ?>';">
															<i class="icofont icofont-copy-alt"></i>
														</button> &nbsp;
														<button type="button" class="btn btn-danger btn-icon" data-toggle="tooltip"  title="ลบ" onclick="deleteWorkflow('<?php echo $rec_main['WF_MAIN_ID']; ?>','<?php echo $rec_main['WF_MAIN_TYPE']; ?>')">
											...
											<!--<input type="hidden" name="total_row<?php echo $i;?>" id="total_row<?php echo $i;?>" value="<?php echo $j;?>">-->
										</tbody>
									</table>
								</div>
							</div>
...
<script src='../assets/js/jquery-sortable.js'></script>
	<script>
		...
		function deleteWorkflow(id, type)
		{
			...
			if(confirm('คุณต้องการลบ Workflow ใช่หรือไม่'))
			{
				...
				/*if(type === "W")
				{
					if(confirm('คุณต้องการ Drop Table ด้วยใช่หรือไม่'))
					{
						window.location.href='workflow_del_function.php?process=delete&drop=Y&id='+id;
					}
					else
					{
						window.location.href='workflow_del_function.php?process=delete&drop=N&id='+id;
					}
				}
				else
				{
		...
		function save_wf_order(id){
			var url = "workflow_arrange_function.php";
