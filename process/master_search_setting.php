...
$sql = db::query("SELECT FORM_QT_ID, 
						...
						FORM_QT_FIELDNAME, 
						FORM_QT_OPERATOR 
				FROM MASTER_CONFIG A
				JOIN FORM_SYSTEM B ON B.FORM_MAIN_ID = A.FORM_QT_INPUT_TYPE   
				WHERE A.WF_MAIN_ID = '".$R["MM_ID"]."' ORDER BY FORM_QT_SORT ASC");
?>
<style>
	.move-td{
		cursor: move;
    ...
    <div class="content-wrapper">
		...
		<div class="row"  id="animationSandbox">
			...
			<div class="col-sm-4">
					...
					<div class="f-right">
						<a class="btn btn-primary waves-effect waves-light" href="<?php echo $p_url; ?>_form.php?W=<?php echo $R["MM_ID"];?>&process=add" role="button">
		...
		<!-- Row end -->
			...
			<form action="<?php echo $p_url; ?>_function.php" method="POST" enctype="multipart/form-data" id="form_wf">
				...
				<input type="hidden" name="W" id="W" value="<?php echo $R['MM_ID']; ?>">
				<!-- Row Starts -->
				<div class="row">
					<div class="col-md-12">
						<div class="card">
										...
										<tbody>
											...
											<tr class="wf_keyword-box">
												...
												<td class="text-center">
													<nobr>
														<button type="button" class="btn btn-warning btn-icon" data-toggle="tooltip" data-placement="top" title="แก้ไข <?php echo $p_name; ?>" onclick="window.location.href='<?php echo $p_url; ?>_form.php?process=edit&FORM_QT_ID=<?php echo $rec['FORM_QT_ID'];?>&W=<?php echo $R["MM_ID"]; ?>';">
															<i class="icofont icofont-edit-alt"></i>
														</button> &nbsp;
														<button type="button" class="btn btn-danger btn-icon" data-toggle="tooltip" data-placement="top" title="ลบ <?php echo $p_name; ?>" onclick="if(confirm('คุณต้องการลบข้อมูลนี้ใช่หรือไม่ ?')){window.location.href='<?php echo $p_url; ?>_function.php?process=delete&FORM_QT_ID=<?php echo $rec['FORM_QT_ID'];?>&W=<?php echo $R["MM_ID"]; ?>';}">
...
<textarea><?php
echo bsl_gen_direction("1","../workflow/workflow_process.php?W=3&WFR=5");
?></textarea>
<br />
