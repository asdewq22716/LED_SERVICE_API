...
$sql_step_form = db::query("SELECT * FROM WF_STEP_FORM WHERE WF_MAIN_ID = '".$R['WF_MAIN_ID']."' ORDER BY WFS_ORDER,WFS_OFFSET");
				...
				//foreach($WF_ARR_FIELD as $key => $value){
					...
					if($field['WFS_FIELD_NAME'] == $R["WF_FIELD_PK"]){
						...
						}elseif($field["FORM_MAIN_ID"] == '11' OR $field["FORM_MAIN_ID"] == '12' OR $field["FORM_MAIN_ID"] == '13'){
							
							$pk = 'true';
							$figure = 'Decision';
							$color = 'redgrad';
							
							if($field["FORM_MAIN_ID"] == '11'){
								$arr_ref_table1[] = 'G_PROVINCE';
								
							}else if($field["FORM_MAIN_ID"] == '12'){
								$arr_ref_table1[] = 'G_AMPHUR';
								
							}else if($field["FORM_MAIN_ID"] == '13'){
		...
		if(count($arr_ref_table) > 0){
			foreach($arr_ref_table as $val){
				...
				$num_rows_wf = db::num_rows($sql_ref_table);
				if($num_rows_wf > 0){
					...
					$REF_ARR_FIELD = db::show_field($val);
					//$ref_data["WF_FIELD_PK"] = get_pk($val);
					$ref_data["WF_MAIN_SHORTNAME"] = $val;
					if($val == 'G_PROVINCE'){
						$ref_data["WF_FIELD_PK"] = 'PROVINCE_CODE';
					}elseif($val == 'G_AMPHUR'){
						$ref_data["WF_FIELD_PK"] = 'AMPHUR_CODE';
					}elseif($val == 'G_TAMBON'){
						$ref_data["WF_FIELD_PK"] = 'TAMBON_CODE';
					}
					$t_other = 'Y';
			...
			<?php
				...
				foreach($REF_ARR_FIELD as $k => $v){
					...
					if($t_other == 'Y'){
						...
						$step_form_ref = select_field_other($ref_data["WF_MAIN_SHORTNAME"], $v);
						//print_r($step_form_ref);
						if($num_rows_ref == $j){
							$comma = '';
						}else{
							$comma = ',';
						...
						if($step_form_ref['FIELD_NAME'] == $ref_data["WF_FIELD_PK"]){
							...
							$figure = 'Decision';
							$color = 'yellowgrad';
							$array_relation[] = '{ from: "'.$R["WF_MAIN_SHORTNAME"].'", to: "'.$ref_data["WF_MAIN_SHORTNAME"].'", text: "1", toText: "1" }';
