	...
	if($rows > 0){
		echo "<ul>";
			while($M = db::fetch_array($sql_menu_group)){
				if($M['MENU_FLAG']=="file"){ $flag="file"; }else{ $flag = "folder"; }
				echo "<li id=\"".$M["MENU_ID"]."\" data-jstree='{\"opened\":true,\"type\":\"".$flag."\"}'>".$M["MENU_NAME"];
				get_menu($M["MENU_ID"]);
				echo "</li>";
			}
		echo "</ul>";
		...
		<div class="container-fluid">
			...
			<div class="row" id="animationSandbox">
				...
				<div class="col-sm-4">
					<div class="f-right">
						<br />
						<button type="button" onClick="save_to_menu();" class="btn  btn-warning waves-effect waves-light"> <i class="icofont icofont-save"></i> Apply เมนูไปยังหน้า Front-end
						</button>
					</div>
				</div>
			</div>
			<!-- Row end -->  
				...
				<div class="row">
					<div class="col-md-6">
						<div class="card">
							<div class="card-header">
								...
								<?php
								$sql_config = db::query("SELECT * FROM WF_CONFIG WHERE CONFIG_NAME = 'wf_show_menu' ");
								$rec_config = db::fetch_array($sql_config); 
								?>
									<input type="checkbox" name="switch" id="switch" class="js-dynamic-lg" data-switchery="true" value="Y" style="display:none" onChange="active_menu(this);" <?php if($rec_config['CONFIG_VALUE'] == "A"){ echo "checked"; } ?>> ใช้งาน
								</h5>
								<div class="f-right">
					...
					<div id="MENU_DETAIL" class="col-md-6">
					</div>
				</div> 
...
$( document ).ready(function() {
  ...
  $('#dragTree').on('changed.jstree', function (e, data) {
	...
	if(menu_id != ""){
		$("#MENU_DETAIL").html('');
		$.get('workflow_menu_edit.php', { 'G' : menu_id, 'parent_id' : data.node.parent, 'text' : data.node.text, 'type' : data.node.type })
			.done(function (html) { 
