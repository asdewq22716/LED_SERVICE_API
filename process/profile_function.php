...
$sql_check1 = db::query("SELECT FIELD_RELETION,FIELD_EDIT,FIELD_NAME FROM USR_SETTING WHERE FIELD_TYPE='O' ORDER BY FIELD_ID");
while($rec_u = db::fetch_array($sql_check1)){
	$USR_OPTION = '';
	if($rec_u["FIELD_RELETION"] == 'M' AND $rec_u["FIELD_EDIT"] == 'Y'){
		$option = '';
