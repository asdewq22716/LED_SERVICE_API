...
$sql = db::query("select * from USR_POSITION ORDER BY POS_ORDER ASC");
?>
<style>
	.move-td{
		cursor: move;
									...
									<table cellspacing="0" id="tech-companies-1" class="table table-bordered sorted_table">
										<thead>
											<tr class="bg-primary">
												...
												<th style="width: 60%;" class="text-center" data-priority="1">ชื่อตำแหน่ง</th>
												...
												<th class="text-center">
													<div class="rkmd-checkbox checkbox-rotate checkbox-ripple">
														<label class="input-checkbox checkbox-success">
															<input type="checkbox" name="POS_STATUS<?php echo $i; ?>" id="POS_STATUS<?php echo $i; ?>" <?php echo $rec['POS_STATUS'] == "Y" ? 'checked' :''; ?> value="Y">
															<span class="checkbox"></span>
														</label>
														<div class="captions"></div>
													</div>
													<input type="hidden" name="id<?php echo $i; ?>" id="id<?php echo $i; ?>" value="<?php echo $rec['POS_ID']; ?>">
												</th>
												<th class="wf_keyword">
													<?php echo $rec['POS_CODE']; ?>
												</th>
												<th class="wf_keyword">
													<?php echo $rec['POS_NAME']; ?>
												</th>
												<th class="wf_keyword">
													<?php echo $rec['POS_SHORT_NAME']; ?>
												</th>
												<td class="text-center move-td">
													<input type="number" id="POS_ORDER<?php echo $i; ?>" name="POS_ORDER<?php echo $i; ?>" value="<?php echo $rec['POS_ORDER']; ?>" class="form-control input-success text-right" style="width:80px;">
												</td>
												<td class="text-center">
													<nobr>
														<button type="button" class="btn btn-warning btn-icon" data-toggle="tooltip" data-placement="top" title="แก้ไข <?php echo $p_name; ?>" onclick="window.location.href='<?php echo $p_url; ?>_form.php?process=edit&G=<?php echo $rec['POS_ID']; ?>';">
															<i class="icofont icofont-edit-alt"></i>
														</button> &nbsp;
														<button type="button" class="btn btn-danger btn-icon" data-toggle="tooltip" data-placement="top" title="ลบ <?php echo $p_name; ?>" onclick="if(confirm('คุณต้องการลบข้อมูลนี้ใช่หรือไม่ ?')){window.location.href='<?php echo $p_url; ?>_function.php?process=delete&id=<?php echo $rec['POS_ID']; ?>';}">
