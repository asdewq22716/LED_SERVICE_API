 ...
 if($data['ResponseCode']['ResCode'] == '000') {	
	...
	for($i=0;$i < $loop;$i++){
		...
		if($num <= 0){
			...
			$Field['ORDER_BOOK_NO'] = $data['Data'][$i]['C12'];
			$Field['ORDER_NO'] = $data['Data'][$i]['C13'];
			$Field['DOC_DATE'] = date_format(date_create($data['Data'][$i]['C14']),"Y-m-d");
			$Field['ACCOUNT_NO'] = $data['Data'][$i]['C15'];
			$Field['ORDER_TO1'] = $data['Data'][$i]['C16'];
			$Field['ORDER_TO2'] = $data['Data'][$i]['C17'];

			
			// print_pre($Field);
			db::db_insert("WH_CIVIL_CASE_ORDER",$Field);
		}
