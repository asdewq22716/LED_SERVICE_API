...
if($filter!=""){
	$sqlSelectData = "	SELECT 		*
						FROM 		WH_BANKRUPT_BALANCE_PERSON 
						WHERE 		1=1 {$filter}"; 


	$querySelectData = db::query($sqlSelectData);
	while($dataSelectData = db::fetch_array($querySelectData)){
		$obj[$i]['registerCode'] 		= $dataSelectData['REGISTER_CODE'];		
		$obj[$i]['fullName'] 			= $dataSelectData['FULL_NAME'];
		$obj[$i]['concernName'] 		= $dataSelectData['CONCERN_NAME'];
		$obj[$i]['courtOrderLog'] 		= $dataSelectData['COURT_ORDER_LOG'];
		$obj[$i]['concernNo'] 			= $dataSelectData['CONCERN_NO'];
