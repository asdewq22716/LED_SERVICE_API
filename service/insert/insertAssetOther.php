...
$data = json_decode($str_json, true);
	...
	$sql = "SELECT COUNT(*) AS NR FROM WH_CIVIL_CASE_ASSETS_OTHER WHERE ASSET_CODE = '".$data['cfcOtherCaptionGen']."'";
	$query = db::query($sql);
	$rec = db::fetch_array($query);
	...
	if($rec['NR'] > 0){
		...
		$update['SEQ'] 	  	         =  $data['seq'];	 
		$update['ASSET_NAME'] 	  	 =  $data['capName'];
		$update['ASSET_AMOUNT'] 	 =  $data['assetAmount'];
		$update['ADDRESS'] 	  	     =  $data['addrNo'].' '.$data['soi'].' '.$data['road'].' '.$data['cen_loc_gen'];
		$update['REGISTER_NO'] 	     =  $data['registerNo'];
     

		$cond = array();
		
		$cond['ASSET_CODE']			 = $data['cfcOtherCaptionGen"'];
		
		db::db_update("WH_CIVIL_CASE_ASSETS_OTHER",$update,$cond);
		...
		$insert['ASSET_CODE']	     = $data['cfcOtherCaptionGen"'];
		...
		$insert['SEQ'] 	  	         =  $data['seq'];	 
		$insert['ASSET_NAME'] 	  	 =  $data['capName'];
		$insert['ASSET_AMOUNT'] 	 =  '';
		$insert['ADDRESS'] 	  	     =  $data['addrNo'].' '.$data['soi'].' '.$data['road'].' '.$data['cen_loc_gen'];
		$insert['REGISTER_NO'] 	     =  $data['registerNo'];
		
		db::db_insert("WH_CIVIL_CASE_ASSETS_OTHER",$insert,'ASSET_ID','ASSET_ID');
