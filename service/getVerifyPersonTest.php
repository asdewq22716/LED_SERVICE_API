...
$request_contents = array(
	...
	$request_contents['personTypeLaw'] = '02';	
	$request_contents['personTypeBr']  = '06';	
	$request_contents['personTypeRv']  = '06';	
	$request_contents['personTypeMd']  = '02';	
		...
		foreach(array_keys($chs) as $key){
			...
			if($data['ResponseCode']['ResCode'] == '000' || $data['statusCode'] == '200'){
				if($key == 'Civil'){
					...
					foreach($data2 as $k2 => $v2){	
						foreach($v2 as $k => $v){
							if(($prefixBlackCase != $v['PREFIX_BLACK_CASE'] || $blackCase != $v['BLACK_CASE'] || $blackYy != $v['BLACK_YY']) && ($prefixRedCase != $v['PREFIX_RED_CASE'] || $redCase != $v['RED_CASE'] || $redYy != $v['RED_YY'] || $courtName !=  $v['COURT_NAME'])){
								$temp = array(
					...
					$data2 = $data['Data'];
					foreach($data2 as $k => $v){	
						if(($prefixBlackCase != $v['prefixBlackCase'] || $blackCase != $v['blackCase'] || $blackYy != $v['blackYY']) && ($prefixRedCase != $v['prefixRedCase'] || $redCase != $v['redCase'] || $redYy != $v['redYY'] || $courtName !=  $v['courtName'])){
								$temp = array(
