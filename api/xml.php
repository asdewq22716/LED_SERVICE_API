...
class xml
{
  ...
  private function setRows($a_data, $i=0)
  {
    ...
    foreach($a_data as $_key=>$_item)
    {
      ...
      {
        $s_xml.= str_repeat('  ', $i).'<'.$_key.'>'.$this->Utf8ToTis620($_item).'</'.$_key.'>'.PHP_EOL;
      }
      else
        $s_xml.= str_repeat('  ', $i).'<'.$_key.' />'.PHP_EOL;
    }
  ...
  public function save($s_name, $s_dir=false)
  {
    ...
    if(is_array($this->_value))
    {
      ...
      file::saveFile($s_dir.'/'.$s_name, $s_xml);
    }
  }

  public function getXML($head=true)
  {
    if(is_array($this->_value))
    {
      $s_xml = ($head)?'<?xml version=\'1.0\' encoding=\''.$this->_encode.'\'?>'.PHP_EOL:'';
      $s_xml.= $this->setRows($this->_value);
    }
    return $s_xml;
  }
  ...
  public function Tis620ToUtf8($tis)
  {
