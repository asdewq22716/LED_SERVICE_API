...
class MediateCmdOfficeAction extends Action
{
    /**
     * {isset(inheritdoc}
     */
    ...
    protected function action(): Response
    {
        ...
        if (!empty($request->cmdTypeName)) {
            $filter .= "AND CMD_TYPE_NAME = '" . $request->cmdTypeName . "'";
        }
        if (!empty($request->officeIdcard)) {
            $filter .= "AND OFFICE_IDCARD = '" . $request->officeIdcard . "'";
        }
        if (!empty($request->officeName)) {
            $filter .= "AND OFFICE_NAME = '" . $request->officeName . "'";
        }
        if (!empty($request->cmdDate)) {
            $filter .= "AND CMD_DATE = '" . $request->cmdDate . "'";
        }

        $sql = \db::query("SELECT * FROM WH_MEDIATE_CMD_OFFICE WHERE 1=1 {$filter}");
        $i = 0;
