...
$TARGET_NAME = conText($_GET['TARGET_NAME']);
$use_select2 = conText($_GET['use_select2']);
...
?>
					...
					<div class="main-header">
						<div class="media m-b-12">
							...
							<div class="media-body">
								...
								<h5>เพิ่ม<?php echo $rec_main['WF_MAIN_NAME']; ?></h5>
							</div>
						</div>
						<div class="f-right">
							<?php
            ...
            <!-- Row end --> 
			<form method="post" enctype="multipart/form-data" id="form_master_mgt" name="form_master_mgt" action="#">
             <!--Workflow row start-->
                <div class="row">
					<div class="col-md-12">
						<div class="card">
							...
							<div class="card-block">
								<div class="form-group row">
									<?php bsf_show_form($W,'0',$WF,$rec_main['WF_TYPE']); ?>
								</div>
							</div>
						</div>
					</div>
...
$('#form_master_mgt').submit(function(){
	var url = "../workflow/master_mgt_function.php"; // the script where you handle the form input.

	$.ajax({
		   type: "POST",
		   url: url,
		   data: $("#form_master_mgt").serialize(), // serializes the form's elements.
		   cache: false,
		   success: function(html)
		   {
				$('#<?php echo $TARGET_NAME; ?>').html('').select2();
				$('#<?php echo $TARGET_NAME; ?>').select2({
				  data: html
				});
				$('select#<?php echo $TARGET_NAME; ?>').trigger("change");
				<?php if($use_select2 != "Y"){ ?>
				$('#<?php echo $TARGET_NAME; ?>').select2("destroy");
				<?php } ?>
				$('#bizModal2').modal('hide');
		   }
		 });
