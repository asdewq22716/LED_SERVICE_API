						...
						<h4><i class="icofont icofont-search-alt-2"></i> ค้นหา</h4>
								...
								<div class="form-group row">
									<div class="col-md-12 text-center">
										...
										&nbsp;&nbsp;
										<button type="button" name="wf_reset" id="wf_reset"  class="btn btn-warning" onClick="window.location.href='report_main.php?W=<?php echo $W; ?>';"><i class="zmdi zmdi-refresh-alt"></i> Reset</button>
...
while($rec = db::fetch_array($sql_cal)){
	...
	$wf_con = " WHERE ".$rec["CAL_START_DATE"]." IS NOT NULL ";
		...
		if($rows_conf == 1){
		$WF_SQL = $WF_EX[0].wf_search_function($W,$_GET,$wf_table,$rec_main["WF_FIELD_PK"])." ORDER BY ".$WF_EX[1];
		}else{
		$WF_SQL = $WF_SQL1;	
		}
	}
	...
	if($WF_SQL != ""){ 
		...
		$title = bsf_show_text($rec["CAL_W_ID"],$rec_r,$rec["CAL_SHOW"],$TYPE);
			$cal_txt .= ",{title: '".$title."',start: '".$rec_r[$date_s]."".$time_s."',end: '".$rec_r[$date_e]."".$time_e."', borderColor: '".$tag_color."', backgroundColor: '".$bg_color."', color: '".$text_color."'".$wf_link."}";
		}
