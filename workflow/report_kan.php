...
<body onload="init()">
	...
	$txt_kan_group = "";
	$txt_kan_list = "";
	$sql_kan_group = db::query("SELECT * FROM WF_KANBAN_GROUP WHERE WF_MAIN_ID = '".$W."' AND K_STATUS = 'Y' ORDER BY K_ORDER ASC");
	while($r_kan_group = db::fetch_array($sql_kan_group)){
		
		$txt_kan_group .= '{"key":"'.$r_kan_group['K_ID'].'", "text":"'.$r_kan_group['K_NAME'].'", "isGroup":true },';
		
		$WF_SQL = "";
		$q_kan_list = db::query("SELECT * FROM WF_KANBAN_LIST WHERE K_ID = '".$r_kan_group['K_ID']."' ORDER BY K_LIST_ID ASC ");
		while($rec = db::fetch_array($q_kan_list)){
			
			if($rec["KAN_W_ID"] != '' AND $rec["KAN_CHOOSE_TYPE"] == '1'){
				
				$sql_m = db::query("select WF_MAIN_ID, WF_MAIN_NAME, WF_MAIN_SHORTNAME, WF_FIELD_PK, WF_TYPE from WF_MAIN where WF_MAIN_ID = '".$rec["KAN_W_ID"]."'");
				...
				$wf_con = " WHERE 1=1 ";
				if(trim($rec["KAN_MORE_SQL"]) != ''){
					$wf_con .= " AND ".wf_convert_var($rec["KAN_MORE_SQL"]);
				}
				/*if($rows_conf == 1){
					$wf_con .= wf_search_function($W,$_GET,$wf_table,$rec_main["WF_FIELD_PK"]);
				}*/
				$WF_SQL = "SELECT * FROM ".$wf_table.$wf_con;
			}else if($rec["KAN_MAIN_SQL"] !=""){
				$WF_SQL1 = htmlspecialchars_decode(wf_convert_var($rec["KAN_MAIN_SQL"]), ENT_QUOTES);
			...
			if($WF_SQL != ""){ 
				...
				while($rec_r = db::fetch_array($sql_r)){
					$txt_kan_link = "";
					if($rec["KAN_LINK"] != ""){
						$txt_kan_link = ",url: '".bsf_show_field($rec["KAN_W_ID"],$rec_r,$rec["KAN_LINK"],$TYPE)."'";
					}
					$title = bsf_show_text($rec["KAN_W_ID"],$rec_r,$rec["KAN_SHOW"],$TYPE);
					$title = preg_replace('/\s\s+/', '\n',$title);
					$title = convert_to_normal_text($title);
					
					$txt_kan_list .= '{"key":"k'.$k.'", "text":"'.$title.'", "group":"'.$r_kan_group['K_ID'].'", "color":"0" },';
					$k++;
				}
			}
		}	
	}
	$kanban_data = $txt_kan_group.substr($txt_kan_list,0,-1);
	?>
	<div id="sample">
		...
		<textarea id="mySavedModel" style="width:100%;height:300px;display:none;">
			...
			"nodeDataArray": [<?php echo $kanban_data;?>],
			"linkDataArray": []}
		</textarea>
	</div>
</body>
