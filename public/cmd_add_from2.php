...
if ($_GET["proc"] == "search_data_add" && $SEND_FROM == 'CIVIL' && $PCC_CIVIL_GEN != '') {
    ...
    $text_param .= "&PCC_CIVIL_GEN=" . $PCC_CIVIL_GEN;



    /* เขียนข้อความ */
    $text_param .= "&receive_prefixBlackCase=" . $_GET['receive_prefixBlackCase'];
    $text_param .= "&receive_blackCase=" . $_GET['receive_blackCase'];
    $text_param .= "&receive_blackYy=" . $_GET['receive_blackYy'];
    $text_param .= "&receive_prefixRedCase=" . $_GET['receive_prefixRedCase'];
    $text_param .= "&receive_redCase=" . $_GET['receive_redCase'];
    $text_param .= "&receive_redYy=" . $_GET['receive_redYy'];
}
...
?>
<script>
    let url = 'cmd_add_from_send_to2.php?<?php echo $text_param; ?>&proc=add'
    // console.log(url)
