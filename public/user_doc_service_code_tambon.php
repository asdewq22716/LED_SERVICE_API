...
$TAMBON_CODE = conText(trim($_GET['TAMBON_CODE']));
$TAMBON_NAME_BOF = conText(trim($_GET['TAMBON_NAME_BOF']));

$filter = pm::genFilter([
  array('TAMBON_CODE', "%$TAMBON_CODE%", 'LIKE'),
  array('TAMBON_NAME_BOF', "%$TAMBON_NAME_BOF%", 'LIKE')
]);

$sql = "SELECT * FROM M_TAMBON_MAP WHERE 1=1 $filter ORDER BY TAMBON_CODE ASC";
$qry = db::query($sql);
      ...
      <div class="col-md-8">
        ...
        <div class="row">
          <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="GET" role="form" enctype="multipart/form-data" style="width: 100%;">
            ...
            <div class="row">
              <?php
              echo pm::boxTxt('TAMBON_CODE', $TAMBON_CODE, 'รหัสตำบล', 6, 4, 8, '');
              echo pm::boxTxt('TAMBON_NAME_BOF', $TAMBON_NAME_BOF, 'ชื่อตำบล', 6, 4, 8, '');
              ?>
            </div>
            <div class="row">
          ...
          <table class="table">
            <thead class="breadcrumb_bg">
              <tr>
                ...
                <th class="text-center">รหัสตำบล</th>
                <th class="text-center">ชื่อตำบล</th>
              </tr>
            </thead>
            <tbody>
              <?php if ($total > 0) {
                ...
                while ($data = db::fetch_array($data_main)) { ?>
                  <tr>
                    ...
                    <td class="text-center"><?php echo $data['TAMBON_CODE']; ?></td>
                    <td><?php echo $data['TAMBON_NAME_BOF']; ?></td>
                  </tr>
                <?php
                  $i++;
                }
        ...
        <div class="table-responsive" hidden>
          <table class="table" id="tambon_table">
            <thead class="breadcrumb_bg">
              <tr>
                <th class="text-center">รหัสตำบล</th>
                <th class="text-center">ชื่อตำบล</th>
...
<script>
  function Export() {
    $("#tambon_table").table2excel({
      filename: "tambon_code"
    });
  }
</script>
<!-- footer part start-->
