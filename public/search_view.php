                  ...
                  <div class="md-group-add-on">
                    ...
                    <div class="md-input-file">
                      ...
                      <span class="md-line"></span>
											...
											<table id="wfsflow-3440" class="table table-bordered sorted_table">
												<thead class="bg-primary">
													<tr class="bg-primary">
														...
														<th style="width:20%;" class="text-center">เจ้าของทรัพย์</th>
														<th style="width:25%;" class="text-center">ประเภทกลุ่มคำสั่งเจ้าพนักงาน</th>
														<th style="width:25%;" class="text-center">คำสั่งเจ้าพนักงาน</th>
														<th style="width:25%;" class="text-center">สภาพทรัพย์</th>
													...
													?>
														<tr>
															...
															<td>
																...
																echo $rec_CMD_TYPE_CODE['ACTION'] == "" ? "ไม่มีข้อมูล" : $rec_CMD_TYPE_CODE['ACTION'];
																?>
															</td>
														</tr>
													<?php
									...
									<div class="col-md-12 wf-center ">
									  ...
									  ";
												...
												<script type="text/javascript">
													/* เมื่อมีเอกสารอัพเข้ามาใหม่ start */
													function loading_doc(num_doc = '<?php echo $num_DOC; ?>') {
													...
													setInterval(loading_doc, 2500);
													/* เมื่อมีเอกสารอัพเข้ามาใหม่ stop */
								...
								?>
										<div class="form-group row">
											...
											<div id="COURT_NAME_BSF_AREA" class="col-md-5 wf-left">
												<label>
													<input type="radio" name="APPROVE_STATUS" id="APPROVE_STATUS1" value="1" checked onclick="func_show_deatial(this.value);"> อนุมัติ
												</label>
												<label>
													<input type="radio" name="APPROVE_STATUS" id="APPROVE_STATUS0" value="0" onclick="func_show_deatial(this.value);"> อนุมัติ/ส่งต่อ
												</label>
												<label>
													<input type="radio" name="APPROVE_STATUS" id="APPROVE_STATUS2" value="2" onclick="func_show_deatial(this.value);"> ส่งกลับแก้ไข
												</label>
											</div>
										</div>

										<div class="row" id="show_note" style="display:none">
											<div id="COURT_NAME_BSF_AREA" class="col-md-2 ">
												<label for="T_COURT_NAME" class="form-control-label wf-right">หมายเหตุ</label>
											</div>
											<div id="COURT_NAME_BSF_AREA" class="col-md-5 wf-left">
												<textarea name="COMMENT_EDIT" id="COMMENT_EDIT" cols="30" rows="10" class="form-control"></textarea>
											</div>
										</div>
										...
										<div class="row">
											...
											<?php
												...
												echo "<font color=\"#FF0000\">หมายเหตุ : ไม่สามารถตอบกลับได้เนื่องจากอนุมัติการตอบกลับคำสั่งเเล้ว<font>";
													...
													?>
													<button type="button" onclick="reply_com('<?php echo  $dataSelectData['ID']; ?>','<?php echo $_GET['TO_PERSON_ID']; ?>');" class="btn btn-success waves-effect waves-light"><i class="icofont icofont-tick-mark" title=""></i> ตอบกลับ</button>
												</div>
											</div>
										</div>
								<?php
								...
								/* stop AK การอนุมัติของเดิม */
								?>

								<script type="text/javascript">
									function func_show_deatial(This) {
										if (This == '1') {
											$('#showSendApprove').hide();
											$('#show_note').hide();
										}
										if (This == '0') {
											$('#showSendApprove').show();
											$('#show_note').hide();
										}
										if (This == '2') {
											$('#showSendApprove').hide();
									...
									$(document).ready(function() {
										...
										var TO_PERSON_ID = '<?php echo $_GET["TO_PERSON_ID"]; ?>'
										...
										$("#btn_save").click(function() {
											...
											// ตรวจสอบค่าที่ได้แล้ว
											if (APPROVE_STATUS == 1) {
												// กรณีเลือก 'อนุมัติ'
												var text_header = "ยืนยันการอนุมัติ";
											} else if (APPROVE_STATUS == 0) {
												// กรณีเลือก 'อนุมัติ/ส่งต่อ'
												var text_header = "ยืนยันการอนุมัติ";
											} else if (APPROVE_STATUS == 2) {
												// กรณีเลือก 'ส่งกลับแก้ไข'
												var text_header = "ยืนยันการส่งกลับแก้ไข";
												...
												function() {
													...
													$.ajax({
														...
														data: {
															...
															ID: <?php echo $_GET["ID"] ?>,
															APPROVE_PERSON_SEND: $('#APPROVE_PERSON_SEND').val(),
															COMMENT_EDIT: COMMENT_EDIT,
															TO_PERSON_ID: TO_PERSON_ID
														},
														cache: false,
														success: function(msg) {
															const D = '<?php echo func::get_E_and_D("search_view", "E", "1=1&SEND_TO=" . $_GET["SEND_TO"] . '&TO_PERSON_ID=' . $_GET["TO_PERSON_ID"]); ?>';
															//self.location.href = 'search_data_cmd.php?SEND_TO=' + SEND_TO + '&TO_PERSON_ID=' + TO_PERSON_ID;
															self.location.href = 'search_data_cmd.php?' + D;
											...
											swal({
												...
												function() {
													...
													$.ajax({
														...
														cache: false,
														success: function(msg) {
															const D = '<?php echo func::get_E_and_D("search_view", "E", "SEND_TO=" . $_GET["SEND_TO"] . '&TO_PERSON_ID=' . $_GET["TO_PERSON_ID"]); ?>';
															//self.location.href = 'search_data_cmd.php?SEND_TO=' + SEND_TO + '&TO_PERSON_ID=' + TO_PERSON_ID;
															self.location.href = 'search_data_cmd.php?' + D;
														}
													});
									...
									function reply_com(REF_ID, TO_PERSON_ID) {

										window.location = 'cmd_add_from_send_to.php?REF_ID=' + REF_ID + '&SEND_TO=<?php echo $_GET["SEND_TO"]; ?>&TO_PERSON_ID=<?php echo $_GET["TO_PERSON_ID"]; ?>'
									}
