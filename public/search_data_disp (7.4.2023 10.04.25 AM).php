							...
							<div class="table-responsive">
								<table cellspacing="0" id="tech-companies-1" class="table table-bordered sorted_table">
									...
									<tbody>
										...
										/* Nop start */
										$sql = db::query("select * from USR_MAIN where USR_ID = '" . $_SESSION["WF_USER_ID"] . "'");
										...
										if ($rec_status['POS_ID'] == '512403') {
											if ($_GET['REGISTERCODE'] != '') {
												/* $fill = "AND  A.REGISTERCODE ='" . $_GET['REGISTERCODE'] . "'"; */
											}
											$fill = "AND  A.REGISTERCODE ='" . $_GET['REGISTERCODE'] . "'";
											$fill1 = "";
										}
										else if ($rec_status['POS_ID'] == '510308') {
											$fill1 = "AND A.APPROVE_PERSON ='" . $rec['USR_USERNAME'] . "'";
										}
										$sqlSelectData = "	SELECT 		TRANSACTION_APPROVE_PERSON,TRANSACTION_STATUS_APP,A.APPROVE_STATUS,A.APPROVE_PERSON,A.ID,CMD_READ_STATUS,CMD_DOC_DATE,CMD_DOC_TIME,CMD_TYPE_NAME,C.FLAG_CMD_NOTI,SERVICE_SYS_NAME,TO_COURT_NAME,A.ID,T_BLACK_CASE,BLACK_CASE,BLACK_YY,T_RED_CASE,RED_CASE,RED_YY,CMD_NOTE,CMD_GRP_NAME ,ALERT_NOTI,SEND_TO,TO_T_BLACK_CASE,TO_BLACK_CASE,TO_BLACK_YY,TO_T_RED_CASE,TO_RED_CASE,TO_RED_YY,CMD_NOTE
															FROM		M_DOC_CMD A
															LEFT JOIN 	M_CMD_SYSTEM B ON A.SYS_NAME = B.CMD_SYSTEM_ID
	...
	/* echo $sqlSelectData; */
										...
										?>
											<tr style="background-color: #E6E6FA;">
												...
												<td align="center">
													<?php
													if ($dataSelectData["APPROVE_STATUS"] == 2 || $dataSelectData["APPROVE_STATUS"] == 0&& $rec_status['POS_ID'] == '512403') {
													?>
														<a href="search_data_from1.php?ID=<?php echo $dataSelectData["ID"] ?>&proc=edit&TO_PERSON_ID=<?php echo $_GET["TO_PERSON_ID"]; ?>&SEND_TO=<?php echo $_GET["SEND_TO"] ?>" class="btn btn-success btn-mini" title=""> <i class="icofont icofont-ui-edit"></i> แก้ไข </a>
													...
													?>
													<a href="search_view.php?ID=<?php echo $dataSelectData["ID"] ?>&update_view=<?php echo $update_view; ?>&TO_PERSON_ID=<?php echo $_GET["TO_PERSON_ID"]; ?>&SEND_TO=<?php echo $_GET["SEND_TO"] ?>" target="_blank" class="btn btn-info btn-mini" title="">
														<i class="icofont icofont-search"></i> ดูรายละเอียด
													</a>
													<?php
													if (  $dataSelectData["APPROVE_STATUS"] == 0 && $rec_status['POS_ID'] == '510308') {// ผอ 
													?>
														<a href="search_view.php?ID=<?php echo $dataSelectData["ID"] ?>&approve=Y&TO_PERSON_ID=<?php echo $_GET["TO_PERSON_ID"]; ?>&SEND_TO=<?php echo $_GET["SEND_TO"]; ?>" target="_blank" class="btn btn-success btn-mini" title="">
															<i class="icofont icofont-search"></i> อนุมัติ
														</a>
													<?php
													}
													/* echo "<pre>";
													print_r($dataSelectData);
													echo "</pre>"; */
			...
			function() {
				...
				$.ajax({
					...
					success: function(html) {
						window.location.href = "search_data_disp.php?SEND_TO=" + $('#SEND_TO').val() + "&REGISTERCODE=" + $('#REGISTERCODE').val();
					}
				});
