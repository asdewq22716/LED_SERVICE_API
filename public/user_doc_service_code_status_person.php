...
$PERSON_MAP_CODE = conText(trim($_GET['PERSON_MAP_CODE']));
$PERSON_NAME = conText(trim($_GET['PERSON_NAME']));

$filter = pm::genFilter([
  array('PERSON_MAP_CODE', "%$PERSON_MAP_CODE%", 'LIKE'),
  array('PERSON_NAME', "%$PERSON_NAME%", 'LIKE')
]);

$sql = "SELECT * FROM M_MAP_PERSON_TYPE WHERE 1=1 $filter ORDER BY PERSON_MAP_CODE ASC";
$qry = db::query($sql);
      ...
      <div class="col-md-8">
        ...
        <div class="row">
          <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="GET" role="form" enctype="multipart/form-data" style="width: 100%;">
            ...
            <div class="row">
              <?php
              echo pm::boxTxt('PERSON_MAP_CODE', $PERSON_MAP_CODE, 'รหัสบุคคล', 6, 4, 8, '');
              echo pm::boxTxt('PERSON_NAME', $PERSON_NAME, 'ชื่อประเภทบุคคล', 6, 4, 8, '');
              ?>
            </div>
            <div class="row">
          ...
          <table class="table">
            <thead class="breadcrumb_bg">
              <tr>
                ...
                <th class="text-center">รหัสบุคคล</th>
                <th class="text-center">ชื่อประเภทบุคคล</th>
              </tr>
            </thead>
            <tbody>
              <?php if ($total > 0) {
                ...
                while ($data = db::fetch_array($data_main)) { ?>
                  <tr>
                    ...
                    <td class="text-center"><?php echo $data['PERSON_MAP_CODE']; ?></td>
        ...
        <div class="table-responsive" hidden>
          <table class="table" id="status_person_table">
            <thead class="breadcrumb_bg">
              <tr>
                <th class="text-center">รหัสบุคคล</th>
                <th class="text-center">ชื่อประเภทบุคคล</th>
...
<script>
  function Export() {
    $("#status_person_table").table2excel({
      filename: "person_status_code"
    });
  }
</script>
<!-- footer part start-->
