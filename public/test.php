...
if($POST["COURT_DATE"]!="")
{
	$filter .= " AND COURT_DATE = '".$POST["COURT_DATE"]."' ";
}
...
$sqlSelectMain = "	select 	* 
					from 	WH_BANKRUPT_CASE_DETAIL
					where 	1=1 {$filter} AND BANKRUPT_CODE IS NOT NULL ORDER BY COURT_DATE DESC , WH_BANKRUPT_ID DESC
				 ";
...
$sqlSelectMain = "	select 	* 
					from 	WH_BANKRUPT_CASE_DETAIL
					where 	1=1  AND BANKRUPT_CODE IS NOT NULL ORDER BY COURT_DATE DESC , WH_BANKRUPT_ID DESC 
				 ";
$querySelectMain = db::query($sqlSelectMain);
while($dataSelectMain = db::fetch_array($querySelectMain)){
	//foreach($arrFieldsCase as $key => $val){
		echo $dataSelectMain['COURT_DATE'].' -> '.$dataSelectMain['BLACK_CASE'].'/'.$dataSelectMain['BLACK_YY'].' '.$dataSelectMain['WH_BANKRUPT_ID'].'  <br>';
	//}
	
}*/
//print_pre($querySelectMain);  
//echo 1;
?>