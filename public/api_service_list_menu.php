<div class="form-group">
  <select class="form-control select2" id="exampleFormControlSelect1" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
    ...
    <optgroup label="ระบบงานบังคับคดีแพ่ง">
      <?php
      $civil_data = db::query("SELECT * FROM M_SERVICE_MANAGE WHERE SYS_DETAIL = 'ระบบงานบังคับคดีแพ่ง' AND SERVICE_STATUS = '1' {$filter} {$filter2}");
    ...
    <optgroup label="ระบบงานบังคับคดีล้มละลาย">
      <?php
      $bankrupt_data = db::query("SELECT * FROM M_SERVICE_MANAGE WHERE SYS_DETAIL = 'ระบบงานบังคับคดีล้มละลาย' AND SERVICE_STATUS = '1' {$filter} {$filter2}");
    ...
    <optgroup label="ระบบงานฟื้นฟูกิจการของลูกหนี้">
      <?php
      $revive_data = db::query("SELECT * FROM M_SERVICE_MANAGE WHERE SYS_DETAIL = 'ระบบงานฟื้นฟูกิจการของลูกหนี้' AND SERVICE_STATUS = '1' {$filter} {$filter2}");
    ...
    <optgroup label="ระบบงานไกล่เกลี่ยข้อพิพาท">
      <?php
      $mediate_data = db::query("SELECT * FROM M_SERVICE_MANAGE WHERE SYS_DETAIL = 'ระบบงานไกล่เกลี่ยข้อพิพาท' AND SERVICE_STATUS = '1' {$filter} {$filter2}");
    ...
    <optgroup label="ระบบ Back office">
      <?php
      $back_data = db::query("SELECT * FROM M_SERVICE_MANAGE WHERE SYS_DETAIL = 'ระบบ Back office' AND SERVICE_STATUS = '1' {$filter} {$filter2}");
