			...
			if($BSF_DET["WFS_FORM_ADD_STATUS"] == "Y"){
				if($BSF_DET["WFS_FORM_ADD_POPUP"] == "Y"){ 
					if($BSF_DET["WFS_FORM_POPUP"] == "P"){
						$WFS_ONCLICK = " onclick=\"PopupCenter('form_mgt.php?W=".$W."&WFS=".$WFS."&WFD=".$WFD."&WFR_ID=".$WFR."&F_TEMP_ID=".$F_TEMP_ID."&WF_POP=P&wfp=".conText($_GET['wfp'])."', '','900','600')\"";
					}else{
						$WFS_ONCLICK = "data-toggle=\"modal\" data-target=\"#bizModal_".$WFS."\" onclick=\"open_modal('form_mgt.php?W=".$W."&WFS=".$WFS."&WFD=".$WFD."&WFR_ID=".$WFR."&F_TEMP_ID=".$F_TEMP_ID."&wfp=".conText($_GET['wfp'])."', '','_".$WFS."')\"";
					}
				?><div class="f-right">
							...
							<a class="btn btn-primary  active waves-effect waves-light" href="#!"  title="<?php if($BSF_DET["WFS_FORM_ADD_RESIZE"] == 'Y'){ echo $WF_TEXT_MAIN_ADD;}?>" onClick="get_wfs_show('wfs_show<?php echo $WFS; ?>','form_add.php','W=<?php echo $W; ?>&WFD=<?php echo $WFD; ?>&WFS=<?php echo $WFS; ?>&WFR=<?php echo $WFR; ?>&F_TEMP_ID=<?php echo $WFR; ?>&wfp=<?php echo conText($_GET['wfp']); ?>','GET','A');"><i class="icofont icofont-ui-add"></i> <?php if($BSF_DET["WFS_FORM_ADD_RESIZE"] != 'Y'){ echo $WF_TEXT_MAIN_ADD;}?></a><small>&nbsp;</small></div><?php 
