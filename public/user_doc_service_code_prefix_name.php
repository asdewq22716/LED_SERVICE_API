...
$P_CODE = conText(trim($_GET['P_CODE']));
$P_NAME_BOF = conText(trim($_GET['P_NAME_BOF']));

$filter = pm::genFilter([
  array('P_CODE', "%$P_CODE%", 'LIKE'),
  array('P_NAME_BOF', "%$P_NAME_BOF%", 'LIKE')
]);

$sql = "SELECT * FROM M_PREFIX_MAP WHERE 1=1 $filter ORDER BY P_ID ASC";
$qry = db::query($sql);
      ...
      <div class="col-md-8">
        ...
        <div class="row">
          <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="GET" role="form" enctype="multipart/form-data" style="width: 100%;">
            ...
            <div class="row">
              <?php
              echo pm::boxTxt('P_CODE', $P_CODE, 'รหัสคำนำหน้าชื่อ', 6, 4, 8, '');
              echo pm::boxTxt('P_NAME_BOF', $P_NAME_BOF, 'คำนำหน้าชื่อ', 6, 4, 8, '');
              ?>
            </div>
            <div class="row">
          ...
          <table class="table">
            <thead class="breadcrumb_bg">
              <tr>
                ...
                <th class="text-center">รหัสคำนำหน้าชื่อ</th>
              ...
              <?php if ($total > 0) {
                ...
                while ($data = db::fetch_array($data_main)) { ?>
                  <tr>
                    ...
                    <td class="text-center"><?php echo $data['P_CODE']; ?></td>
                    <td><?php echo $data['P_NAME_BOF']; ?></td>
                  </tr>
                <?php
                  $i++;
                }
        ...
        <div class="table-responsive" hidden>
          <table class="table" id="prefix_table">
            <thead class="breadcrumb_bg">
              <tr>
                <th class="text-center">รหัสคำนำหน้าชื่อ</th>
                <th class="text-center">คำนำหน้าชื่อ</th>
...
<script>
  function Export() {
    $("#prefix_table").table2excel({
      filename: "prefix_name_code"
    });
  }
</script>
<!-- footer part start-->
