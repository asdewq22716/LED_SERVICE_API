						...
						<form method="get" id="form_wf_search" name="form_wf_search" action="#">
							...
							<div class="form-group row">
							  ...
							  <div class="form-group row">
								...
								<div id="CASE_TYPE_BSF_AREA" class="col-md-2 wf-left">
									<select name="S_COM_STATUS" id="S_COM_STATUS"   class="form-control select2 select2-hidden-accessible" tabindex="-1" aria-hidden="true"> 
							...
							<table cellspacing="0" id="tech-companies-1" class="table table-bordered sorted_table">
								...
								<tbody>
									...
									$orderBy = "";
									if($tab_active==1){
										$filterCom = " AND A.SEND_TO = '".$_GET["SEND_TO"]."' and (A.TO_PERSON_ID = '".$_GET["TO_PERSON_ID"]."' or A.TRANSACTION_APPROVE_PERSON = '".$_GET["TO_PERSON_ID"]."') ";
										if($S_COM_STATUS==1){
											$filterCom .= " AND CMD_READ_STATUS = '0' ";
										}
										$filterCom .= " AND APPROVE_STATUS = 1";
										$orderBy = "ORDER BY CMD_READ_STATUS ASC,CMD_DOC_DATE ASC,CMD_DOC_TIME ASC";
									}else if($tab_active==2){
										$filterCom = " AND A.SYS_NAME = '".$_GET["SEND_TO"]."' AND ( A.OFFICE_IDCARD = '".$_GET["TO_PERSON_ID"]."' OR APPROVE_PERSON = '".$_GET["TO_PERSON_ID"]."') ";
										$orderBy = "ORDER BY (CASE WHEN APPROVE_STATUS = 0 AND APPROVE_PERSON = '".$_GET["TO_PERSON_ID"]."' THEN 1 ELSE 2 END) ASC ,CMD_READ_STATUS ASC,CMD_DOC_DATE ASC,CMD_DOC_TIME ASC";
									}
									if($T_NO_BLACK!=""){
									...
									$sqlSelectData = "	SELECT 		TRANSACTION_APPROVE_PERSON,TRANSACTION_STATUS_APP,A.APPROVE_STATUS,A.APPROVE_PERSON,A.ID,CMD_READ_STATUS,CMD_DOC_DATE,CMD_DOC_TIME,CMD_TYPE_NAME,SERVICE_SYS_NAME,COURT_NAME,A.ID,T_BLACK_CASE,BLACK_CASE,BLACK_YY,T_RED_CASE,RED_CASE,RED_YY,CMD_NOTE,CMD_GRP_NAME ,ALERT_NOTI
														...
														WHERE		1=1 {$filterCom} AND ROWNUM < 20
														{$orderBy}
																	"; 
