...
$SERVICE_SYSTEM = ($_GET['SERVICE_SYSTEM']) ? conText($_GET['SERVICE_SYSTEM']) : '';
...
$filterSystem = "";
if ($SERVICE_SYSTEM != '') {
	$filterSystem = " AND CMD_SYSTEM_ID = '$SERVICE_SYSTEM' ";
}

$sqlSystem = "SELECT * FROM $M_CMD_SYSTEM WHERE CMD_SYSTEM_ID IN (1,2,3,4) {$filterSystem} ORDER BY CASE WHEN CMD_SYSTEM_ID = 4 THEN 1 WHEN CMD_SYSTEM_ID = 3 THEN 2 WHEN CMD_SYSTEM_ID = 1 THEN 3 ELSE 4  END";
...
while ($recSystem = db::fetch_assoc($qrySystem)) {
	...
	$sqlServiceMain = "SELECT * FROM $M_SERVICE_MANAGE WHERE 1 = 1 AND SERVICE_STATUS = '1'AND SYS_NAME = '{$recSystem['CMD_SYSTEM_ID']}' ORDER BY SERVICE_CODE ASC ";
