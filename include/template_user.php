...
<div id="sys_loading" style="position:fixed; left:0px; top:0px; z-index:5000; width:100%; height:100%; display:none; background:rgba(1,1,1,0.7); text-align:center; color:#FFFFFF;">
...
<style>
	...
	<?php if ($CONF_HEADER_LOGO_STYLE != '') { ?>@media (min-width: 768px) {
		.navbar-static-top {
			<?php if ($CONF_HEADER_LOGO_STYLE != '') {
				echo substr($CONF_HEADER_LOGO_STYLE, 0, -1) . ' !important;';
			} ?>
		}

		.main-header-top>a {
			<?php if ($CONF_HEADER_LOGO_WIDTH != '') {
				echo substr($CONF_HEADER_LOGO_WIDTH, 0, -1) . ' !important;';
			} ?>
		}
	}

	<?php } ?><?php if ($CONF_HEADER_LOGO_WIDTH != '') { ?>@media (max-width: 767px) {
		.main-header-top a {
			text-align: left;
